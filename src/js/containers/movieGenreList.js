...
class MovieGenreList extends Component {
    ...
    async componentWillMount() {
        let arr=[];
        let pro=[];
        const genres = await this.props.getMovieGenres();
        const moviesbygenre = await genres.data.map((elem) => {return (this.props.getMoviesByGenre(elem.id)).then(() => {
            arr.push(elem);
...
function matchDispatchToProps(dispatch) {
    return bindActionCreators({getMovieGenres: getMovieGenres, getMoviesByGenre: getMoviesByGenre,updateAllMoviesGenres: updateAllMoviesGenres}, dispatch);
}
